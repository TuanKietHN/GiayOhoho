      ...
      <Grid container spacing={2}>
        {orders.map(o => (
          <Grid item xs={12} md={6} key={o.id}>
            <Card onClick={() => nav(`/orders/${o.id}`)} style={{ cursor: 'pointer' }}>
              <CardContent>
                <Typography variant="subtitle1">#{o.id} • {({pending:'đang chờ giao hàng',paid:'đã thanh toán',shipping:'đang giao',done:'hoàn tất',cancel:'đã hủy'})[o.status] || o.status}</Typography>
