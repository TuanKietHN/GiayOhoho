...
class OrderController extends Controller
{
    public function checkout(Request $request)
    {
        ...
        $cart = Cart::with(['items.variant.product', 'coupon'])
        ...
        // giao dịch
        return DB::transaction(function () use ($userId, $sub, $discount, $total, $coupon, $orderAddress, $cart) {
    ...
    {
        $orders = OrderDetail::with(['items.variant.product'])
    ...
    {
        $order = OrderDetail::with(['items.variant.product'])
            ->where('user_id', $request->user()->id)
            ->findOrFail($id);
